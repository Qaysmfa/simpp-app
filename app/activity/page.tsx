...
export default function ActivityPage() {
  ...
  return <div className="min-h-screen flex" style={{ background: "#F3F5F8" }}><Sidebar role={user.role} view="activity" onNavigate={(key) => router.push(key === "cases" ? "/dashboard" : `/${key}`)} mobileOpen={false} setMobileOpen={() => undefined} /><div className="flex-1 min-w-0"><Header title="Activity Log" user={user} setMobileOpen={() => undefined} /><main className="p-4 lg:p-6">{error ? <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded px-3 py-2">{error}</div> : <div className="bg-white border rounded-md overflow-x-auto" style={{ borderColor: "#DFE3E8" }}><table className="w-full text-sm"><thead><tr className="text-left text-gray-500 border-b bg-[#FAFBFC]">{["Pengguna", "Aksi", "No. Perkara", "Nilai Sebelumnya", "Nilai Baru", "Waktu"].map((heading) => <th key={heading} className="px-4 py-3 font-medium whitespace-nowrap">{heading}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-b last:border-0"><td className="px-4 py-3">{row.user_name}</td><td className="px-4 py-3">{row.action}</td><td className="px-4 py-3">{row.case_number || "-"}</td><td className="px-4 py-3 text-gray-500">{row.previous_value || "-"}</td><td className="px-4 py-3 text-gray-500">{row.new_value || "-"}</td><td className="px-4 py-3 text-gray-500 whitespace-nowrap">{new Date(row.created_at).toLocaleString("id-ID")}</td></tr>)}</tbody></table></div>}</main></div></div>;
}
