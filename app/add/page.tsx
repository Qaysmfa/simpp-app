...
export default function AddCasePage() {
  ...
  return <div className="min-h-screen flex" style={{ background: "#F3F5F8" }}><Sidebar role={user.role} view="add" onNavigate={(key) => router.push(key === "cases" ? "/cases" : `/${key}`)} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} /><div className="flex-1 min-w-0"><Header title="Tambah Jadwal Pembacaan" user={user} setMobileOpen={setMobileOpen} /><main className="p-4 lg:p-6 space-y-4 max-w-4xl">{error && <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded px-3 py-2">{error}</div>}<CaseForm onCancel={() => router.push("/cases")} onSave={(form) => void save(form)} /></main></div></div>;
}
